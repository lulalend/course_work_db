do $$
    begin
        ...
        if not exists (select 1 from pg_type where typname = 'sex_') then
            create type sex_ as enum ('male', 'female', 'other');
        end if;
        ...
        if not exists (select 1 from pg_type where typname = 'trip_status') then
            create type trip_status as enum
                ('draft', 'waiting_driver', 'waiting_passenger', 'in_progress', 'finished');
        end if;
end $$;

--create type transport_type as enum ('car', 'motorbike', 'ship', 'plane');
create table if not exists transport_make (
                                              id serial primary key,
                                              make varchar(25) not null,

                                              constraint make_unique unique (make)
);
...
create table if not exists transport_details (
                                                 ...
                                                 constraint details_unique unique (model_id, type),
...
create table if not exists transport (
                                         ...
                                         constraint transport_details_unique
                                             unique (transport_details_id, year, country, reg_number, available_seats),
...
create table if not exists drivers_license (
                                               ...
                                               constraint license unique (license_number),
...
create table if not exists driver_transport (
                                                ...
                                                constraint transport_unique unique (driver_id, transport_id),
...
create table if not exists location (
                                        ...
                                        constraint location_unique unique (name, latitude, longitude)
);
...
create table if not exists trip (
                                    ...
                                    constraint arrivial_location_fk
